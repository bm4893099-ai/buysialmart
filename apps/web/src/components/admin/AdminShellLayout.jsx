        ...
        <aside className={drawerClassName}>
          ...
          <div className="px-5 py-5">
            <GlassPanel className="p-4">
              <p className={isDark ? 'text-xs uppercase tracking-[0.22em] text-indigo-200' : 'text-xs uppercase tracking-[0.22em] text-indigo-600'}>{t('adminShell.controlCenter')}</p>
              <h2 className={isDark ? 'mt-3 text-lg font-semibold text-white' : 'mt-3 text-lg font-semibold text-slate-900'}>{session.user.name || session.user.email}</h2>
              <p className={isDark ? 'mt-2 text-sm text-slate-300' : 'mt-2 text-sm text-slate-600'}>{session.user.role === USER_ROLES.SUPER_ADMIN ? t('adminShell.superAdminAccess') : t('adminShell.adminAccess')}</p>
            </GlassPanel>
          </div>
