...
export default function ZatcaQrPreview({ sellerName, vatNumber }) {
  ...
  return (
    <GlassPanel className="p-6">
      <p className="text-xs uppercase tracking-[0.24em] text-indigo-200">{t('superAdmin.sampleQr')}</p>
      <div className="mt-4 flex flex-col gap-6 sm:flex-row sm:items-center">
        ...
        <div className="flex-1">
          <h3 className="text-lg font-semibold text-white">{sellerName || t('superAdmin.defaultSellerName')}</h3>
          <p className="mt-2 text-sm leading-7 text-slate-300">{t('superAdmin.qrHint')}</p>
          <div className="mt-4 rounded-2xl bg-slate-950/70 p-4 text-xs leading-6 text-slate-400">
            <span className="block text-slate-200">{qrValue ? `${qrValue.slice(0, 72)}...` : t('superAdmin.awaitingQrInput')}</span>
          </div>
        </div>
      </div>
    </GlassPanel>
